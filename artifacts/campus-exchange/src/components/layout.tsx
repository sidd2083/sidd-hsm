...
import { ChevronDown, Menu, X, LogOut, Wallet, BarChart2, Settings, Trophy, Info } from "lucide-react";
  ...
  return (
    <Link href={href}>
      <span className={cn(
        ...
        (active || isHome) ? "text-gray-900" : "text-gray-400 hover:text-gray-700"
      )}>
        {label}
        {(active || isHome) && (
          ...
          {user && me ? (
            <>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 bg-white border border-[#E8EAF0] rounded-xl p-1.5" style={{ boxShadow: "0 8px 24px rgba(0,0,0,0.10)" }}>
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/about" className="cursor-pointer flex items-center gap-2.5 rounded-lg px-3 py-2 hover:bg-slate-50 text-sm font-medium text-gray-700">
        ...
        <div className="md:hidden border-t border-[#E8EAF0] bg-white px-4 py-3 space-y-1">
          {[
            ...
            <Link key={item.href} href={item.href}>
              <div className="px-3 py-2.5 text-sm font-semibold text-gray-700 rounded-lg hover:bg-slate-50 cursor-pointer" onClick={() => setMobileOpen(false)}>
                {item.label}
              </div>
            </Link>
          ))}
      ...
      <footer className="border-t border-[#E8EAF0] bg-white mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-300 font-medium">
            <Link href="/about"><span className="hover:text-gray-400 transition-colors cursor-pointer">About</span></Link>
            <Link href="/leaderboard"><span className="hover:text-gray-400 transition-colors cursor-pointer">Leaderboard</span></Link>
