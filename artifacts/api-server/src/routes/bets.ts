...
router.post("/bets", requireAuth, async (req: AuthRequest, res) => {
  ...
  const bet = await db().runTransaction(async (txn) => {
    ...
    if (market["status"] !== "active") {
      throw Object.assign(new Error("Market is locked or resolved"), {
        status: 409,
      });
    }
    ...
    txn.update(marketRef, {
      [poolField]: admin.firestore.FieldValue.increment(amountPaid),
    });
...
router.get("/bets/my", requireAuth, async (req: AuthRequest, res) => {
  ...
  const marketIds = [
    ...new Set(snapshot.docs.map((d) => d.data()["marketId"] as string)),
  ];
